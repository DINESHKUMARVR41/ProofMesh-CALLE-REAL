...
export async function GET(req:Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(attempt?.transcript_turns){const turns=attempt.transcript_turns as Array<{speaker?:string;text?:string;offset_seconds?:number}>;inv.transcript=turns.map(x=>({role:x.speaker==='bot'?'agent':'human',text:x.text??'',timestampSeconds:x.offset_seconds??0}))}
  await saveCase(c)
