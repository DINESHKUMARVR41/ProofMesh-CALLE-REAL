...
export async function geminiClaims(doc:SourceDocument,text:string):Promise<Claim[]>{
 ...
 return arr.filter(isRawClaim).filter(x=>isClaimType(x.type)&&typeof x.predicate==='string'&&x.predicate.length>0&&x.value!==undefined).map(x=>({id:crypto.randomUUID(),sourceId:doc.id,sourceName:doc.name,type:x.type,subject:String(x.subject||'document'),predicate:String(x.predicate),value:typeof x.value==='number'?x.value:String(x.value),unit:x.unit?String(x.unit):undefined,confidence:Math.max(0,Math.min(1,Number(x.confidence??.8))),extractedBy:'gemini' as const}))
}
