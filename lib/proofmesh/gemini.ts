...
export async function geminiClaims(doc:SourceDocument,text:string):Promise<Claim[]>{
 ...
 const j=await r.json();const raw=j.candidates?.[0]?.content?.parts?.map((p:any)=>p.text||'').join('')||'[]';let arr:any[]=[];try{arr=JSON.parse(raw)}catch{arr=[]}
 const allowed=new Set(['entity','quantity','price','total','date','validity','identifier','text'])
 return arr.filter(x=>x&&allowed.has(x.type)&&x.predicate&&x.value!==undefined).map(x=>({id:crypto.randomUUID(),sourceId:doc.id,sourceName:doc.name,type:x.type,subject:String(x.subject||'document'),predicate:String(x.predicate),value:typeof x.value==='number'?x.value:String(x.value),unit:x.unit?String(x.unit):undefined,confidence:Math.max(0,Math.min(1,Number(x.confidence??.8))),extractedBy:'gemini' as const}))
}
