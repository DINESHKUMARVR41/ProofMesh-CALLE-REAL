...
export interface AnalysisMetadata { mode:'gemini'|'rules'|'mixed'; documents:Array<{documentId:string;name:string;method:'gemini'|'rules';geminiError?:string}>; completedAt:string }
export interface ProofCase { id:string; name:string; status:'ready'|'investigating'|'resolved'; createdAt:string; documents:SourceDocument[]; claims:Claim[]; nodes:GraphNode[]; edges:GraphEdge[]; findings:Finding[]; investigations:Investigation[]; summary:{totalClaims:number;findings:number;unresolved:number;verified:number}; analysis?:AnalysisMetadata }
