export type EvidenceKind = 'purchase_order' | 'invoice' | 'delivery_note' | 'certificate' | 'company_profile' | 'other'
export type ClaimType = 'entity' | 'quantity' | 'price' | 'total' | 'date' | 'validity' | 'identifier' | 'text'
...
export interface Claim { id:string; sourceId:string; sourceName:string; type:ClaimType; subject:string; predicate:string; value:string|number; unit?:string; page?:number; confidence:number }
...
export interface Finding { id:string; type:string; title:string; severity:FindingSeverity; status:FindingStatus; explanation:string; claimIds:string[]; missingEvidence:string[]; createdAt:string; resolution?:string }
